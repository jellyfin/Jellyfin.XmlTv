...
public class XmlTvReaderTests
{
    ...
    public void SchedulesDirectTest()
    {
        ...
        // Pick a channel to check the data for
        var channel = reader.GetChannels().SingleOrDefault(c => c.Id == "EPG123.10108.schedulesdirect.org"); // CTV Toronto
        Assert.NotNull(channel);
        ...
        Assert.NotNull(programme!.Episode);
        Assert.Equal(9, programme.Credits?.Count);
        Assert.Equal(4, programme.Icons?.Count);
    ...
    public void ImageIconTest()
    {
        ...
        var startDate = new DateTimeOffset(2020, 07, 29, 0, 0, 0, new TimeSpan());
        var programme = reader.GetProgrammes(channel!.Id, startDate, startDate.AddDays(1)).SingleOrDefault(p => p.Title == "Unforgettable");
        Assert.NotNull(programme);
        Assert.Equal(4, programme.Images?.Count);
        Assert.Equal(5, programme.Icons?.Count);

        var credits = programme.Credits;
        Assert.NotNull(credits);
        Assert.Equal(30, credits?.Count);
        Assert.Equal(1, credits![5]?.Images?.Count);
    ...
    public void UrlTest()
    {
        ...
        var channel = reader.GetChannels().SingleOrDefault(c => c.Id == "EPG123.10108.schedulesdirect.org"); // CTV Toronto
        Assert.NotNull(channel);
        Assert.Equal(1, channel.Urls?.Count);
        var url = channel.Urls?.First();
        Assert.NotNull(url);
        Assert.Equal("https://example.com/channel_one", url.Uri);
        Assert.Equal("imdb", url.System);
        ...
        // Programme
        Assert.NotNull(programme);
        url = programme.Urls?.First();
        Assert.NotNull(url);
        Assert.Equal("https://example.com/programme", url.Uri);
        Assert.Equal("imdb", url.System);
        ...
        Assert.NotNull(credits);
        Assert.Equal(30, credits?.Count);
        var urls = credits![5]?.Urls;
        Assert.Equal(2, urls?.Count);
        url = urls?[1];
        Assert.NotNull(url);
        Assert.Equal("https://example.com/person/205", url.Uri);
        Assert.Equal("imdb", url.System);
    }
}
